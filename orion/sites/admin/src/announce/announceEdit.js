...
export class AnnounceEditPage extends React.Component {
    ...
    componentDidMount() {
      ...
      if (announceId) {
        API.get("api/announcements/v1/announcement/" + announceId)
          .then(res => {
    ...
    onClickSave() {
        ...
        let failCallback = (err) => alert("Could not save announcement: " + err.response.data);
        if (this.state.isEdit) {
            API.post("api/announcements/v1/announcement/" + this.state.announceId, announcement)
    ...
    onConfirmDelete() {
      const announceId = this.props.announceId;
      API.delete("api/announcements/v1/announcement/" + announceId)
          .then(res => {
              window.location.hash = "announcements";
          })
    }
      ...
      render() {
        const title = this.state.isEdit ? "Edit Announcement" : "Add Announcement";
        ...
        if (this.state.showSaveModal) {
          ...
          modalContent = <OkayModal text={"Announcement information saved!"}
        ...
        return (
            ...
            <h4>Message</h4>
            <textarea value={this.state.inputMessage}
