...
func Test_PublishSessions(t *testing.T) {
	...
	// Get All Unpublished
	recorder11 := sendHttpRequest(t, http.MethodGet, "/api/sessions/v1/unpublished", nil)

	// Validate results
	assert.EqualValues(t, http.StatusOK, recorder11.Code)
	var ids []uint
	if err := json.Unmarshal(recorder11.Body.Bytes(), &ids); err != nil {
		t.Errorf("unexpected error: %v\n", err)
	}
	assert.EqualValues(t, 1, ids[0])
	assert.EqualValues(t, 2, ids[1])
	assert.EqualValues(t, 3, ids[2])
	assert.EqualValues(t, 3, len(ids))
	...
	recorder13 := sendHttpRequest(t, http.MethodGet, "/api/sessions/v1/unpublished", nil)

	// Validate results
	assert.EqualValues(t, http.StatusOK, recorder13.Code)
	if err := json.Unmarshal(recorder13.Body.Bytes(), &ids); err != nil {
		t.Errorf("unexpected error: %v\n", err)
	}
