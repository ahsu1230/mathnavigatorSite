...
// Test: Create 3 Classes and GetClassesByProgram()
func Test_GetClassesByProgram(t *testing.T) {
	...
	recorder4 := sendHttpRequest(t, http.MethodGet, "/api/classes/v1/classes/program/program1", nil)
	...
	assert.EqualValues(t, "2020_summer", classes[1].SemesterId)
	assert.EqualValues(t, sql.NullString{}, classes[1].ClassKey)
	assert.EqualValues(t, "program1_2020_summer", classes[1].ClassId)
	assert.EqualValues(t, "churchill", classes[1].LocationId)
	assert.EqualValues(t, "5 pm - 7 pm", classes[1].Times)
	assert.EqualValues(t, now, classes[1].StartDate)
	assert.EqualValues(t, now.Add(time.Hour*24*30), classes[1].EndDate)
...
// Test: Create 3 Classes and GetClassesBySemester()
func Test_GetClassesBySemester(t *testing.T) {
	...
	recorder4 := sendHttpRequest(t, http.MethodGet, "/api/classes/v1/classes/semester/2020_summer", nil)
	...
	assert.EqualValues(t, "program3", classes[1].ProgramId)
	assert.EqualValues(t, "2020_summer", classes[1].SemesterId)
	assert.EqualValues(t, sql.NullString{String: "class2", Valid: true}, classes[1].ClassKey)
	assert.EqualValues(t, "program3_2020_summer_class2", classes[1].ClassId)
	assert.EqualValues(t, "churchill", classes[1].LocationId)
	assert.EqualValues(t, "5 pm - 7 pm", classes[1].Times)
	assert.EqualValues(t, now.Add(time.Hour*24*60), classes[1].StartDate)
	assert.EqualValues(t, now.Add(time.Hour*24*120), classes[1].EndDate)
...
func Test_GetClassesByProgramAndSemester(t *testing.T) {
	...
	class3 := createClass(
		"program1",
		"2020_summer",
		sql.NullString{String: "final_review", Valid: true},
		"program1_2020_summer_final_review",
		"churchill",
		"5 pm - 8 pm",
		now.Add(time.Hour*24*30),
		now.Add(time.Hour*24*31),
	)
	class4 := createClass(
		"program2",
		"2020_summer",
		...
		"4 pm - 6 pm",
		now,
		now.Add(time.Hour*24*90),
	)

	body1 := createJsonBody(class1)
	body2 := createJsonBody(class2)
	body3 := createJsonBody(class3)
	body4 := createJsonBody(class4)
	recorder1 := sendHttpRequest(t, http.MethodPost, "/api/classes/v1/create", body1)
	recorder2 := sendHttpRequest(t, http.MethodPost, "/api/classes/v1/create", body2)
	recorder3 := sendHttpRequest(t, http.MethodPost, "/api/classes/v1/create", body3)
	...
	recorder5 := sendHttpRequest(t, http.MethodGet, "/api/classes/v1/classes/program/program1/semester/2020_summer", nil)

	// Validate results
	assert.EqualValues(t, http.StatusOK, recorder5.Code)
	var classes []domains.Class
	if err := json.Unmarshal(recorder5.Body.Bytes(), &classes); err != nil {
		t.Errorf("unexpected error: %v\n", err)
	}
