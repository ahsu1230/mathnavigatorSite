...
var ERR_MYSQL_FOREIGN_KEY_CHILD_CONSTRAINT_FAILURE = errors.New("MYSQL_FOREIGN_KEY_CHILD_CONSTRAINT_ERROR")
var ERR_MYSQL_FOREIGN_KEY_PARENT_CONSTRAINT_FAILURE = errors.New("MYSQL_FOREIGN_KEY_PARENT_CONSTRAINT_ERROR")
...
func checkSqlError(e error, statement string, v ...interface{}) (error, bool) {
	...
	if ok {
		if me.Number == 1062 {
		...
		} else if me.Number == 1451 {
			return errors.Wrapf(
				ERR_MYSQL_FOREIGN_KEY_PARENT_CONSTRAINT_FAILURE,
				"Foreign key constraint failure (%v) on parent when executing SQL statement (%s) with args (%v)",
				me,
				statement,
				v,
			), true
			...
			return errors.Wrapf(
				ERR_MYSQL_FOREIGN_KEY_CHILD_CONSTRAINT_FAILURE,
				"Foreign key constraint failure (%v) on child when executing SQL statement (%s) with args (%v)",
				me,
				statement,
				v,
			), true
