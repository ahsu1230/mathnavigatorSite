...
func TestValidFirstName(t *testing.T) {
	...
	user := domains.User{
		...
		FamilyId:   domains.NewNullUint(0),
	}
...
//tests valid Family ID
func TestValidFamilyId(t *testing.T) {
	// Checks for valid family ids
	user := domains.User{
		FirstName:  "John",
		LastName:   "Smith",
		Email:      "gmail@gmail.com",
		...
		IsGuardian: false,
		GuardianId: domains.NewNullUint(2),
		FamilyId:   domains.NewNullUint(1),
	}
	if err := user.Validate(); err != nil {
		t.Errorf("Check was incorrect, got: %s, expected: nil", err.Error())
	}
	...
	// Checks for invalid guardian ids
	user.FamilyId = domains.NewNullUint(0)
	if err := user.Validate(); err == nil {
		t.Error("Check was incorrect, got: nil, expected: invalid family id")
	}
}
